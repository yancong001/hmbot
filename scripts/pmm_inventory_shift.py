...
class PMMInventoryShift(ScriptStrategyBase):
    ...
    def get_candles_with_features(self):
        ...
        candles_df['bid_spread_bps'] = candles_df[f"NATR_{self.candles_length}"] * self.bid_spread_scalar * 100
        self.logger().info(f"Log bid_spread_bps {candles_df['bid_spread_bps']}")
        candles_df['ask_spread_bps'] = candles_df[f"NATR_{self.candles_length}"] * self.ask_spread_scalar * 100
        self.logger().info(f"Log ask_spread_bps {candles_df['ask_spread_bps']}")
...
def update_multipliers(self):
    ...
    self.logger().info(f"Log candles_df {candles_df}")
...
def format_status(self) -> str:
    ...
    lines.extend([
        f"  Bid Spread (bps): {self.bid_spread * 10000:.4f} | Best Bid Spread (bps): {best_bid_spread * 10000:.4f}"])
    lines.extend([
        f"  Ask Spread (bps): {self.ask_spread * 10000:.4f} | Best Ask Spread (bps): {best_ask_spread * 10000:.4f}"])
    lines.extend(["\n----------------------------------------------------------------------\n"])
    lines.extend(["  Price Shifts:"])
    lines.extend([f"  Max Shift (bps): {self.max_shift_spread * 10000:.4f}"])
    lines.extend([
        f"  Trend Scalar: {self.trend_scalar:.1f} | Trend Multiplier (bps): {self.price_multiplier * 10000:.4f} | Trend Price Shift: {trend_price_shift:.4f}"])
    lines.extend([
        f"  Target Inventory Ratio: {self.target_ratio:.4f} | Current Inventory Ratio: {self.current_ratio:.4f} | Inventory Delta: {self.inventory_delta:.4f}"])
    lines.extend([
        f"  Inventory Multiplier (bps): {self.inventory_multiplier * 10000:.4f} | Inventory Price Shift: {inventory_price_shift:.4f}"])
