...
from typing import Awaitable, Dict, List, NamedTuple, Optional, Union
...
class BitgetExchangeTests(unittest.TestCase):
    # the level is required to receive logs from the data source logger
    level = 0

    market: BitgetExchange
    ...
    def setUpClass(cls) -> None:
        ...
        print("Initializing Bitget market... this will take about a minute.")
    ...
    def setUp(self) -> None:
        ...
        self.exchange = BitgetExchange(
            ...
            bitget_secret_key=self.api_secret_key,
            bitget_passphrase=self.api_passphrase,
    ...
    def _validate_auth_credentials_present(self, request_data: NamedTuple):
        pass

    @aioresponses()
    def test_all_trading_pairs(self, mock_api):
        self.exchange._set_trading_pair_symbol_map(None)
        url = web_utils.public_rest_url(path_url=CONSTANTS.SYMBOL_PATH_URL)
        resp = {
            "data": [
                {
                    "symbol": self.trading_pair,
    ...
    def test_get_last_traded_prices(self, mock_api):
        ...
        url1 = web_utils.public_rest_url(path_url=CONSTANTS.LATEST_SYMBOL_INFORMATION_ENDPOINT)
        url1 = f"{url1}?symbol=BTCUSDT"
        ...
        url2 = web_utils.public_rest_url(path_url=CONSTANTS.LATEST_SYMBOL_INFORMATION_ENDPOINT)
        url2 = f"{url2}?symbol=BTCUSDT"
        ...
        ret = self.async_run_with_timeout(
            coroutine=self.exchange.get_last_traded_prices(["BTCUSDT"])
        )
        ...
        self.assertEqual(ret["BTCUSDT"], 0.03715004)
    ...
    def test_get_fee_returns_fee_from_exchange_if_available_and_default_if_not(self, mocked_api):
        url = web_utils.public_rest_url(CONSTANTS.TRADING_FEE_QUERY_PATH_URL)
        ...
        self.assertEqual(Decimal("0.002"), fee.percent)  # default fee
    ...
    def test_create_limit_order_successfully(self, mock_api):
        ...
        url = web_utils.private_rest_url(CONSTANTS.ORDER_CREATE_PATH_URL)
        regex_url = re.compile(f"^{url}".replace(".", r"\.").replace("?", r"\?"))
    ...
    @aioresponses()
    @patch("hummingbot.connector.exchange.bitget.bitget_exchange.BitgetExchange.get_price")
    ...
    def test_cancel_order_successfully(self, mock_api):
        ...
        url = web_utils.get_rest_url_for_endpoint(endpoint=CONSTANTS.ORDER_DELETE_PATH_URL)
        regex_url = re.compile(f"^{url}".replace(".", r"\.").replace("?", r"\?") + ".*")
        response = {
            "code": "00000",
            "data": {
        ...
        cancel_request = next(((key, value) for key, value in mock_api.requests.items()
                               if key[1].human_repr().startswith(url)))
        print(f"cancel_request",cancel_request)
